...
def _miniapp_shell(title: str, body: str, active: str = '') -> str:
  classes = {k: ('active' if active == k else '') for k in ('home','manuals','numbers','profile')}
  ...
  * {{ box-sizing:border-box; -webkit-tap-highlight-color:transparent; }} html,body {{ margin:0; padding:0; min-height:100%; background:radial-gradient(circle at top right, rgba(180,28,36,.20) 0%, rgba(180,28,36,0) 28%),radial-gradient(circle at top, rgba(255,190,92,.12) 0%, rgba(255,190,92,0) 30%),linear-gradient(180deg, #17100f 0%, var(--bg2) 34%, var(--bg) 100%); color:var(--text); font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; overscroll-behavior:none; touch-action:pan-x pan-y; }}
  ...
  </style></head><body><div class=\"wrap\">{body}</div><div class=\"bottomnav\"><a class=\"navbtn {classes['home']}\" href=\"/\"><span>🏠</span><span>Главная</span></a><a class=\"navbtn {classes['manuals']}\" href=\"/manuals\"><span>📚</span><span>Мануалы</span></a><a class=\"navbtn {classes['numbers']}\" href=\"/numbers\"><span>📦</span><span>Номера</span></a><a class=\"navbtn {classes['profile']}\" href=\"/profile\"><span>👤</span><span>Профиль</span></a></div><script>window.Telegram?.WebApp?.ready();window.Telegram?.WebApp?.expand();document.addEventListener('gesturestart',e=>e.preventDefault());</script></body></html>"""


def miniapp_profile_html() -> str:
  body = """<div class=\"top\"><div class=\"head\"><small>Diamond Vault Esim</small><h1>Профиль</h1><p>Данные аккаунта, баланс и живая сводка по работе.</p></div><a class=\"chip\" href=\"/\">DVE</a></div><div class=\"box pad\"><div style=\"display:flex;align-items:center;gap:14px;margin-bottom:14px;\"><div id=\"pf_avatar\" style=\"width:62px;height:62px;border-radius:18px;background:linear-gradient(135deg,rgba(191,40,52,.95),rgba(69,18,19,.98));display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:900;color:#fff;box-shadow:0 10px 24px rgba(0,0,0,.22);\">D</div><div style=\"min-width:0;flex:1;\"><div id=\"pf_name\" style=\"font-size:20px;font-weight:800;color:#f3dfb1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;\">—</div><div id=\"pf_tag\" style=\"font-size:14px;color:#d3b072;margin-top:3px;\">—</div><div id=\"pf_id\" style=\"font-size:13px;color:#9f8454;margin-top:4px;\">ID: —</div></div></div><div class=\"grid\"><div class=\"info\"><h3>Баланс</h3><div id=\"pf_balance\">—</div></div><div class=\"info\"><h3>Заработано</h3><div id=\"pf_earned\">—</div></div><div class=\"info\"><h3>Всего сдано</h3><div id=\"pf_total\">—</div></div><div class=\"info\"><h3>Успешно</h3><div id=\"pf_completed\">—</div></div><div class=\"info\"><h3>В очереди</h3><div id=\"pf_queue\">—</div></div><div class=\"info\"><h3>Рефералы</h3><div id=\"pf_refs\">—</div></div></div><div style=\"display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px;\"><a class=\"chip\" style=\"justify-content:center;min-height:48px;\" href=\"/numbers\">📦 Мои номера</a><a class=\"chip\" style=\"justify-content:center;min-height:48px;\" href=\"/manuals\">📚 Мануалы</a></div></div><script>const tg=window.Telegram?.WebApp;const u=tg?.initDataUnsafe?.user;function setTxt(id,val){const el=document.getElementById(id);if(el)el.textContent=val;}if(u){const full=[u.first_name,u.last_name].filter(Boolean).join(' ')||'Пользователь';setTxt('pf_name',full);setTxt('pf_tag',u.username?('@'+u.username):'Без username');setTxt('pf_id','ID: '+u.id);const avImg=document.getElementById('pf_avatar_img');const avFallback=document.getElementById('pf_avatar_fallback');if(u.photo_url&&avImg){avImg.src=u.photo_url;avImg.style.display='block';if(avFallback)avFallback.style.display='none';}else if(avFallback){avFallback.textContent=(u.first_name||'D').trim().slice(0,1).toUpperCase();}fetch('/api/profile-summary?user_id='+encodeURIComponent(u.id)).then(r=>r.json()).then(d=>{setTxt('pf_balance',d.balance||'$0');setTxt('pf_total',String(d.total??0));setTxt('pf_completed',String(d.completed??0));setTxt('pf_earned',d.earned||'$0');setTxt('pf_refs',String(d.refs??0));setTxt('pf_queue',String(d.current_queue??0));}).catch(()=>{setTxt('pf_balance','$0');setTxt('pf_total','0');setTxt('pf_completed','0');setTxt('pf_earned','$0');setTxt('pf_refs','0');setTxt('pf_queue','0');});} </script>"""
  return _miniapp_shell('Профиль', body, 'profile')


def miniapp_numbers_html() -> str:
  body = """<div class=\"top\"><div class=\"head\"><small>Diamond Vault Esim</small><h1>Мои номера</h1><p>Активные заявки, статусы и текущая очередь.</p></div><a class=\"chip\" href=\"/\">DVE</a></div><div class=\"box pad\"><div style=\"display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px;\"><div class=\"chip\" id=\"nm_total\">Всего: 0</div><div class=\"chip\" id=\"nm_active\">Активных: 0</div><button class=\"chip\" id=\"nm_reload\" type=\"button\" style=\"cursor:pointer;\">Обновить</button></div><div id=\"numbersWrap\" class=\"list\"><div class=\"empty\">Загрузка заявок…</div></div></div><script>const u=window.Telegram?.WebApp?.initDataUnsafe?.user;const wrap=document.getElementById('numbersWrap');const totalEl=document.getElementById('nm_total');const activeEl=document.getElementById('nm_active');function colorByOperator(op){const s=(op||'').toLowerCase();if(s.includes('мтс'))return 'rgba(180,28,39,.16)';if(s.includes('билайн'))return 'rgba(206,175,28,.16)';if(s.includes('втб')||s.includes('газ'))return 'rgba(38,95,224,.16)';return 'rgba(239,198,112,.08)';}function renderItems(items){totalEl.textContent='Всего: '+items.length;activeEl.textContent='Активных: '+items.length;if(!items.length){wrap.innerHTML='<div class=\"empty\">Сейчас активных заявок нет.</div>';return;}wrap.innerHTML=items.map(it=>`<div class=\"row\" style=\"background:${colorByOperator(it.operator)};border:1px solid rgba(236,194,107,.14);border-radius:18px;padding:14px 14px 12px;\"><div style=\"display:flex;align-items:center;justify-content:space-between;gap:10px;\"><b>#${it.id} • ${it.operator}</b><span class=\"chip\" style=\"font-size:12px;padding:6px 10px;\">${it.mode}</span></div><div style=\"margin-top:8px;font-size:15px;color:#f3dfb1;\">${it.phone}</div><div style=\"margin-top:6px;font-size:13px;color:#d0b072;\">${it.status}${it.position?(' • позиция '+it.position):''}</div></div>`).join('');}function loadItems(){if(!u){wrap.innerHTML='<div class=\"empty\">Откройте mini app из Telegram.</div>';return;}fetch('/api/my-numbers?user_id='+encodeURIComponent(u.id)).then(r=>r.json()).then(d=>renderItems(d.items||[])).catch(()=>{wrap.innerHTML='<div class=\"empty\">Не удалось загрузить заявки.</div>';});}document.getElementById('nm_reload').addEventListener('click',loadItems);loadItems();setInterval(loadItems,12000);</script>"""
  return _miniapp_shell('Мои номера', body, 'numbers')


def miniapp_submit_html(bot_username: str) -> str:
  body = """<div class="top"><div class="head"><small>Diamond Vault Esim</small><h1>Сдать eSIM</h1><p>Отправка номера и QR напрямую через mini app.</p></div><a class="chip" href="/">DVE</a></div><div class="box pad"><div class="info" style="margin-bottom:12px;"><h3>Новая заявка</h3><div>Заполни данные ниже. После отправки заявка сразу появится в той же очереди бота.</div></div><form id="submitForm" style="display:grid;gap:12px;"><select id="sb_operator" required style="width:100%;min-height:54px;border-radius:18px;padding:0 16px;background:#120d0b;border:1px solid rgba(236,194,107,.18);color:#f5e8c6;font-size:16px;"><option value="mts">🔴 МТС</option><option value="mts_premium">🔴 МТС Салон</option><option value="bil">🟡 Билайн</option><option value="mega">🟢 Мегафон</option><option value="t2">⚫ Tele2</option><option value="vtb">🔵 ВТБ</option><option value="gaz">🔵 Газпром</option></select><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><button type="button" class="chip" id="mode_hold" data-mode="hold" style="justify-content:center;min-height:52px;background:linear-gradient(135deg, rgba(117,16,23,.96), rgba(59,16,18,.98));">⏳ Холд</button><button type="button" class="chip" id="mode_nohold" data-mode="no_hold" style="justify-content:center;min-height:52px;">⚡ Безхолд</button></div><input id="sb_phone" type="tel" inputmode="numeric" placeholder="Номер в формате +7XXXXXXXXXX" required style="width:100%;min-height:54px;border-radius:18px;padding:0 16px;background:#120d0b;border:1px solid rgba(236,194,107,.18);color:#f5e8c6;font-size:16px;"><label class="chip" style="justify-content:center;min-height:58px;cursor:pointer;">🖼 Загрузить QR<input id="sb_qr" type="file" accept="image/*" required style="display:none"></label><div id="sb_preview" class="empty">QR ещё не выбран.</div><button id="sb_submit" type="submit" class="chip" style="justify-content:center;min-height:58px;background:linear-gradient(135deg, rgba(143,14,23,.98), rgba(74,19,21,.99));font-size:18px;font-weight:900;">📲 Сдать eSIM</button><div id="sb_msg" class="empty"></div></form></div><script>const tg=window.Telegram?.WebApp;const u=tg?.initDataUnsafe?.user;let submitMode='hold';const holdBtn=document.getElementById('mode_hold');const noholdBtn=document.getElementById('mode_nohold');function syncMode(){holdBtn.style.opacity=submitMode==='hold'?'1':'.62';noholdBtn.style.opacity=submitMode==='no_hold'?'1':'.62';}holdBtn.onclick=()=>{submitMode='hold';syncMode();};noholdBtn.onclick=()=>{submitMode='no_hold';syncMode();};syncMode();const qr=document.getElementById('sb_qr');const preview=document.getElementById('sb_preview');qr.addEventListener('change',()=>{const f=qr.files?.[0];preview.textContent=f?('Выбран файл: '+f.name):'QR ещё не выбран.';});document.getElementById('submitForm').addEventListener('submit', async (e)=>{e.preventDefault();const msg=document.getElementById('sb_msg');msg.textContent='Отправка...';if(!u){msg.textContent='Открой mini app из Telegram.';return;}const f=qr.files?.[0];if(!f){msg.textContent='Загрузи QR.';return;}const fd=new FormData();fd.append('user_id',String(u.id));fd.append('username',u.username||'');fd.append('full_name',[u.first_name||'',u.last_name||''].join(' ').trim());fd.append('operator_key',document.getElementById('sb_operator').value);fd.append('mode',submitMode);fd.append('phone',document.getElementById('sb_phone').value);fd.append('qr',f,f.name||'qr.jpg');try{const r=await fetch('/api/submit-esim',{method:'POST',body:fd});const d=await r.json();if(!r.ok||!d.ok){msg.textContent=d.error||'Не удалось отправить заявку.';return;}msg.textContent='Заявка #'+d.item_id+' отправлена.';document.getElementById('sb_phone').value='';qr.value='';preview.textContent='QR ещё не выбран.';setTimeout(()=>{location.href='/numbers';},700);}catch(err){msg.textContent='Ошибка отправки.';}});</script>"""
