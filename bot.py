...
def miniapp_home_html(bot_username: str) -> str:
  ...
  <div class="box pad" style="margin-top:14px;"><small class="eyebrow">Быстрый доступ</small><div class="grid2"><a class="btn primary" style="min-height:118px;align-items:flex-start;flex-direction:column;justify-content:flex-start;padding-top:16px;" href="__SUBMIT__"><span style="font-size:28px;">📲</span><span style="font-size:22px;">Сдать eSIM</span><span class="muted" style="font-weight:500;line-height:1.35">Создать новую заявку и перейти к отправке QR.</span></a><a class="btn gold" style="min-height:118px;align-items:flex-start;flex-direction:column;justify-content:flex-start;padding-top:16px;" href="/manuals"><span style="font-size:28px;">📚</span><span style="font-size:22px;">Мануалы</span><span class="muted" style="font-weight:500;line-height:1.35">Открыть библиотеку материалов и разделы по операторам.</span></a><a class="btn blue" style="min-height:118px;align-items:flex-start;flex-direction:column;justify-content:flex-start;padding-top:16px;" href="/profile"><span style="font-size:28px;">👤</span><span style="font-size:22px;">Профиль</span><span class="muted" style="font-weight:500;line-height:1.35">Тег, ID, баланс и статистика аккаунта.</span></a><a class="btn secondary" style="min-height:118px;align-items:flex-start;flex-direction:column;justify-content:flex-start;padding-top:16px;" href="/numbers"><span style="font-size:28px;">📦</span><span style="font-size:22px;">Мои номера</span><span class="muted" style="font-weight:500;line-height:1.35">Список заявок, статусы и место в очереди.</span></a></div></div>
...
def miniapp_submit_html(bot_username: str) -> str:
  ...
  <div class="box pad" id="submitRoot">
    ...
    <h1 class="h1" style="font-size:38px;">Сдать eSIM</h1>
    <p class="lead">Заполни заявку быстро и отправь QR прямо через mini app.</p>
    <div style="display:grid;grid-template-columns:1.15fr .85fr .85fr;gap:10px;margin-top:14px;" id="metaGrid">
      <div class="info"><h3>Прайс</h3><div id="livePrice">—</div></div>
      <div class="info"><h3>Очередь</h3><div id="liveQueue">—</div></div>
      <div class="info"><h3>Статус</h3><div id="liveStatus">—</div></div>
    </div>
    <form id="submitForm" style="margin-top:12px;display:grid;gap:10px;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <select id="sb_operator" class="input"><option value="mts">🔴 МТС</option><option value="beeline">🟡 Билайн</option><option value="vtb">🔵 ВТБ</option><option value="gazprom">🔵 Газпром</option><option value="megafon">🟢 Мегафон</option><option value="tele2">⚫ Tele2</option></select>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><button type="button" id="modeHold" class="btn primary" style="justify-content:center;min-height:56px;">⏳ Холд</button><button type="button" id="modeNoHold" class="btn secondary" style="justify-content:center;min-height:56px;">⚡ Безхолд</button></div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr auto;gap:10px;"><button type="button" class="btn gold" id="pickQr" style="justify-content:center;min-height:56px;">🖼 Загрузить QR</button><div class="empty" id="qrPreview" style="min-height:56px;display:flex;align-items:center;justify-content:center;padding:0 12px;">QR ещё не выбран.</div></div>
      <button type="submit" class="btn primary" id="sbSubmit" style="justify-content:center;min-height:58px;font-size:28px;">📲 Сдать eSIM</button>
  ...
  const modeHold=document.getElementById('modeHold'); const modeNoHold=document.getElementById('modeNoHold'); function paintMode(){ if(submitMode==='hold'){modeHold.className='btn primary';modeNoHold.className='btn secondary';} else {modeHold.className='btn secondary';modeNoHold.className='btn primary';} }
...
def miniapp_profile_html() -> str:
  body = """<div class="box pad"><small class="eyebrow">Профиль</small><h1 class="h1" style="font-size:38px;">Профиль</h1><p class="lead">Тег, ID, баланс и живая сводка по аккаунту.</p><div style="display:flex;align-items:center;gap:14px;margin-top:14px;margin-bottom:14px;"><div id="pf_avatar" style="width:68px;height:68px;border-radius:20px;background:linear-gradient(135deg,rgba(191,40,52,.95),rgba(69,18,19,.98));display:flex;align-items:center;justify-content:center;font-size:28px;font-weight:900;color:#fff;overflow:hidden;"><img id="pf_avatar_img" style="width:100%;height:100%;object-fit:cover;display:none;"><div id="pf_avatar_fallback">D</div></div><div style="min-width:0;flex:1;"><div id="pf_name" style="font-size:20px;font-weight:800;color:#f3dfb1;">—</div><div id="pf_tag" style="font-size:14px;color:#d3b072;margin-top:3px;">—</div><div id="pf_id" style="font-size:13px;color:#9f8454;margin-top:4px;">ID: —</div></div></div><div class="info-grid"><div class="info"><h3>Баланс</h3><div id="pf_balance">—</div></div><div class="info"><h3>Заработано</h3><div id="pf_earned">—</div></div><div class="info"><h3>Всего сдано</h3><div id="pf_total">—</div></div><div class="info"><h3>В очереди</h3><div id="pf_queue">—</div></div></div><div class="grid2" style="margin-top:12px;"><a class="btn secondary" href="/numbers">📦 Мои номера</a><a class="btn gold" href="/manuals">📚 Мануалы</a></div><div class="box pad" style="margin-top:14px;"><small class="eyebrow">Последние действия</small><div id="pf_recent" class="empty">Загрузка...</div></div></div><script>const u=window.Telegram?.WebApp?.initDataUnsafe?.user||{};function esc(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}function fmt(v){if(!v)return '—';const d=new Date(v.replace(' ','T'));if(isNaN(d.getTime()))return v;return d.toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});}async function loadProfile(){if(!u?.id){document.getElementById('pf_recent').textContent='Открой mini app из Telegram.';return;}const r=await fetch('/api/profile-summary?user_id='+encodeURIComponent(u.id));const d=await r.json();document.getElementById('pf_name').textContent=[u.first_name||'',u.last_name||''].join(' ').trim()||'Пользователь';document.getElementById('pf_tag').textContent=u.username?('@'+u.username):'—';document.getElementById('pf_id').textContent='ID: '+(u.id||'—');document.getElementById('pf_balance').textContent=d.balance||'—';document.getElementById('pf_earned').textContent=d.earned||'—';document.getElementById('pf_total').textContent=String(d.total||0);document.getElementById('pf_queue').textContent=String(d.current_queue||0);const img=document.getElementById('pf_avatar_img');const fb=document.getElementById('pf_avatar_fallback');if(u.photo_url){img.src=u.photo_url;img.style.display='block';fb.style.display='none';}const recent=(d.recent||[]).map(it=>`<div class="info" style="margin-top:8px;"><div style="font-weight:800;color:#f3dfb1;">${esc(it.operator)} · ${esc(it.status)}</div><div class="muted" style="margin-top:4px;">${esc(it.phone||'—')} · ${esc(it.mode||'—')} · ${esc(fmt(it.created_at))}</div></div>`).join('');document.getElementById('pf_recent').innerHTML=recent||'Нет данных.';}loadProfile();</script>"""
