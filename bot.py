...
def _miniapp_shell(title: str, body: str, active: str = '') -> str:
  ...
  * {{ box-sizing:border-box; -webkit-tap-highlight-color:transparent; }} html,body {{ margin:0; padding:0; min-height:100%; background:radial-gradient(circle at top right, rgba(180,28,36,.20) 0%, rgba(180,28,36,0) 28%),radial-gradient(circle at top, rgba(255,190,92,.12) 0%, rgba(255,190,92,0) 30%),linear-gradient(180deg, #17100f 0%, var(--bg2) 34%, var(--bg) 100%); color:var(--text); font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; overscroll-behavior:none; touch-action:pan-x pan-y; }}
  ...
  .bottomnav {{ position:fixed; left:50%; bottom:10px; transform:translateX(-50%); width:min(calc(100% - 18px), 720px); display:grid; grid-template-columns:repeat(4,1fr); gap:8px; padding:10px; border-radius:24px; border:1px solid var(--line); background:rgba(11,8,7,.92); backdrop-filter:blur(10px); box-shadow:0 10px 24px rgba(0,0,0,.35); }} .navbtn {{ display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:58px; border-radius:16px; color:#f5e8c6; font-size:12px; gap:4px; background:linear-gradient(180deg, rgba(30,20,15,.96), rgba(14,10,9,.98)); border:1px solid rgba(234,196,116,.16); text-decoration:none; }} .navbtn.active {{ outline:1px solid rgba(234,196,116,.32); color:var(--gold2); }}
...
def miniapp_submit_html(bot_username: str) -> str:
  bot_link = f"https://t.me/{bot_username}" if bot_username else "https://t.me/"
  body = f"""<div class="top"><div class="head"><small>Diamond Vault Esim</small><h1>Сдать eSIM</h1><p>Переход в основной бот для отправки номера и QR.</p></div><a class="chip" href="/">DVE</a></div><div class="box pad"><div class="info" style="margin-bottom:12px;"><h3>Быстрый доступ</h3><div>Открой бота и продолжай сдачу в основной очереди без лишних переходов.</div></div><div style="display:grid;grid-template-columns:1fr;gap:10px;"><a class="chip" style="justify-content:center;min-height:54px;background:linear-gradient(135deg, rgba(117,16,23,.96), rgba(59,16,18,.98));" href="{bot_link}">Открыть бота для сдачи</a><a class="chip" style="justify-content:center;min-height:54px;" href="/numbers">Перейти в Мои номера</a></div></div>"""
  return _miniapp_shell('Сдать eSIM', body, '')
